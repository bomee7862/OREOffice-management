...
// 정산 상세 데이터 계산 (GET 핸들러 및 확정 시 공용)
async function getSettlementDetail(year_month: string) {
...
router.get('/:year_month', async (req, res) => {
  try {
    ...
    const detail = await getSettlementDetail(year_month);
    res.json(detail);
  } catch (error) {
...
router.post('/:year_month/confirm', async (req, res) => {
  try {
    ...
    // 상세 데이터를 JSON으로 저장
    const detailData = await getSettlementDetail(year_month);
